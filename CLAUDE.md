...
- **`delete: false`** on all collections — content is never deleted via CMS. Decap commits deletes and unpublishes directly to `main`, which the branch protection rule rejects. Alerts are taken down with the `draft` field ("Hide from website"), an ordinary edit that goes through the editorial workflow
